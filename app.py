...
# --- 1. 核心資料與設定 ---
# 提示：此處使用 Firebase RTDB，已根據需求設定為公開存取（不需要授權）
        ...
        try:
            ...
            if db_data:
                ...
                for o_id in filtered_orders:
                    o_df = df[df["製令"] == o_id]
                    # 安全獲取日期
                    latest_row = o_df.sort_values("提交時間", ascending=False).iloc[0]
                    p_date = latest_row.get("通電日期", "未設定")

                    if s_staff != "全部":
                        check_cols = ["組長"] + [f"人員{i}" for i in range(1, 6) if f"人員{i}" in o_df.columns]
                        if not o_df[check_cols].apply(lambda x: s_staff in x.values, axis=1).any(): 
                            continue
            ...
            st.error("❌ 連線資料庫失敗，請確認 Firebase 設定。")
            ...
            if submit_clicked:
                try:
                    ...
                    if target_key:
                        ...
                        res = requests.post(f"{DB_URL}.json", data=json.dumps(payload), timeout=5)
