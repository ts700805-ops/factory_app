...
if "user" not in st.session_state:
    ...
    with st.container(border=True):
        # 管理員與人員都在同一個下拉式選單
        target_user = st.selectbox("請選擇姓名", list(user_list.keys()))
        input_code = st.text_input("輸入代碼", type="password")
        if st.button("確認進入", use_container_width=True):
            if user_list.get(target_user) == input_code:
    ...
    if st.session_state.user == "管理員":
        st.header("📊 數位戰情室看板")
        logs = get_from_db('production_logs')
        if logs:
        ...
        st.divider()
        # 帳號管理區：解決 image_dd3ce0.png 提到的寫入問題
        st.subheader("👤 帳號管理員 (新增人員)")
        with st.container(border=True):
            c_u, c_p = st.columns(2)
            n_name = c_u.text_input("新員工姓名")
            n_code = c_p.text_input("新員工代碼")
            if st.button("➕ 建立帳號並同步"):
                if n_name and n_code:
                    try:
                        db.reference(f'users/{n_name}').set(n_code)
                        st.success(f"✅ 帳號「{n_name}」已同步！請點「登出」後即可看到新名單。")
                    except Exception as e:
                        st.error(f"連線不穩定，請重按一次。錯誤：{e}")
        st.divider()
