...
# --- 📜 完工紀錄查詢 (原功能完全保留，一律不修改) ---
        ...
        # 使用 Tab 分離回報與管理
        tab_report, tab_admin = st.tabs(["📝 進行回報", "⚙️ 人員與組別管理"])

        # --- 設定管理邏輯 ---
        with tab_admin:
            st.subheader("⚙️ 設定組長與對應組員")
            st.info("格式：組長姓名:組員1,組員2,組員3 (每組一行)")
            
            # 初始資料 (若無後台資料則用此預設)
            default_text = "陳德文:徐梓翔,牟育玄,林建安,魏瑄毅,羅立昕,江金福,呂是儒,邱信維,張瑀榛,陳宛廷,戴鎰祥,鍾明志,黃瑞翎,羅文發,羅章淳,蕭桓惟,周棟榮,李偉誠,潘信成,周政龍,傑米,達文,吉爾\n劉志偉:劉定澤,胡瑄芸,蕭詩瓊,劉秀鳳,龍才華,龍斯愷,姜治銘,彭毓萱,邱珍娜,陳建勳,黃建堃,麥可,費南,阿杰"
            
            # 建立編輯框
            config_text = st.text_area("編輯區", default_text, height=200, key="admin_config_input")
            ...
            if st.button("💾 儲存人員配置"):
                ...
                for line in config_text.splitlines():
                    if ":" in line:
                        ...
                        new_map[leader.strip()] = [m.strip() for m in members.split(",")]
                
                # 存入 Session 以便當下更新
                st.session_state.temp_mapping = new_map
                st.success("✅ 設定已更新，請切換回「進行回報」頁籤即可使用最新名單！")

        # --- 回報流程 ---
        with tab_report:
            # 讀取設定 (優先使用管理者儲存的，否則使用預設)
            mapping = st.session_state.get("temp_mapping", {
            ...
            with col_l:
                selected_leader = st.selectbox("👤 選擇所屬組長：", list(mapping.keys()), key="rep_leader_sel")
            with col_m:
                members = mapping.get(selected_leader, [])
                selected_user = st.selectbox("🎯 選擇回報同仁姓名：", members, key="rep_user_sel")
            
            # --- 您的原有回報按鈕與邏輯 ---
            # ... (此處放置您原本的 if st.button(...) 提交程式碼) ...
            st.warning(f"準備為【{selected_user}】提交今日 6S 回報")
    # ==========================================
    # 🎮 6S 戰境養成功能區塊
    # ==========================================
