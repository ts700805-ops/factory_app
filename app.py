...
if "user" not in st.session_state:
                ...
                if all_logs:
                    ...
                    if curr:
                        with st.expander("📝 編輯內容", expanded=True):
                            ...
                            if st.button("💾 儲存派工修改"):
                                patch_data = {"製令": edit_order, "製造工序": edit_process, "派工人員": edit_assigner, "作業人員": new_worker, "協助人員": new_assist}
    ...
    # --- 7. ⚙️ 系統內容管理 ---
        ...
        with st.form("basic_settings"):
            st.subheader("1. 基礎名單設定")
            new_orders = st.text_area("📦 編輯製令清單 (以逗號隔開)", value=",".join(settings.get("orders", [])), height=100)
            new_assigners = st.text_area("🚩 編輯派工人員清單 (以逗號隔開)", value=",".join(settings.get("assigners", [])), height=100)
            new_processes = st.text_area("⚙️ 編輯製造工序清單 (以逗號隔開)", value=",".join(settings.get("processes", ["預設工序"])), height=100)
        ...
        with st.form("worker_config"):
            worker_input = st.text_area(f"👷 編輯『{target_assigner}』的作業人員清單 (以逗號隔開)", value=",".join(current_workers), height=150)
            if st.form_submit_button(f"💾 儲存 {target_assigner} 的作業員配置"):
                ...
                st.success(f"{target_assigner} 的人員清單已更新！")
                st.rerun()
