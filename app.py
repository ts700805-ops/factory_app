...
DONE_URL = "https://my-factory-system-default-rtdb.firebaseio.com/completed_logs" # 新增：完工資料存放區
        ...
        try:
            ...
            if db_data:
                ...
                with st.container():
                    ...
                    # 讓人員選擇哪一筆完工
                    target_task = col_sel.selectbox("請選擇已完成的製令/人員", 
                                                    options=filtered_df.index,
                                                    format_func=lambda i: f"{filtered_df.loc[i, '製令']} - {filtered_df.loc[i, '作業人員']} ({filtered_df.loc[i, '製造工序']})")
                    
                    if col_btn.button("確認完工"):
                        try:
                            selected_row = filtered_df.loc[target_task].to_dict()
                            task_id = selected_row.pop('id') # 取得資料庫 key 並移除
                            selected_row['實際完工時間'] = get_now_str()
                            
                            # 1. 搬移到完工區
                            post_r = requests.post(f"{DONE_URL}.json", json=selected_row)
                            if post_r.status_code == 200:
                                # 2. 從待辦區刪除
                                requests.delete(f"{DB_URL}/{task_id}.json")
                                st.balloons()
                                st.success(f"製令 {selected_row['製令']} 已記錄完工！")
                                st.rerun()
                ...
                with st.expander("📝 編輯此筆內容", expanded=False):
                    ...
                    if st.button("💾 儲存修改"):
                        requests.patch(f"{DB_URL}/{target_id}.json", json={
                            "製令": new_order, "製造工序": new_proc, "派工人員": new_a, "作業人員": new_w, "作業期限": str(new_d)
                        })
                        st.balloons()
                        st.success("紀錄已成功更新！")
                        st.rerun()
