...
# ==========================================
  ...
  # 更改為獨立 if 判定，徹底解決 elif 造成的 SyntaxError 語法錯誤
       ...
       # ==========================================
        ...
        if not all_opponents:
            ...
            <div style="color:#ffffff !important; font-family:sans-serif; background-color: #4C1D95; border: 4px solid #2E1065; padding: 20px; border-radius: 12px;">
                    ...
                    while hp1 > 0 and hp2 > 0 and round_num <= 10:
                        ...
                        hp2 -= dmg1
                        logs_html += f"⚔️ <b>第 {round_num} 回合</b>：【{p1_title}】{current_user} 攻擊，造成 <span style='color:#7E22CE;'><b>{dmg1}</b></span> 點傷害！<br>"
                        if hp2 <= 0: break
                        
                        dmg2 = max(1, p2_atk + random.randint(-3, 3))
                        hp1 -= dmg2
                        logs_html += f"🛡️ <b>第 {round_num} 回合</b>：【{p2_title}】{target_user} 反擊，造成 <span style='color:#7E22CE;'><b>{dmg2}</b></span> 點傷害！<br>"
        ...
        # ==========================================
        # 👑 稱號查詢與升級門檻對照區 (依組長與同仁姓名查詢)
        # ==========================================
        st.markdown("---")
        st.markdown("### <span style='color:#FBBF24 !important;'>👑 稱號查詢與升級門檻對照區</span>", unsafe_allow_html=True)
        
        tc1, tc2 = st.columns(2)
        with tc1:
            st.markdown("##### <span style='color:#FBBF24 !important;'>📌 升級門檻與稱號對照表</span>", unsafe_allow_html=True)
            st.markdown(f"""
            - <span style='color:#FBBF24 !important;'>**{t_list[0]}**：總點數 0 ~ 9 點 (初始階級)</span>
            - <span style='color:#FBBF24 !important;'>**{t_list[1]}**：總點數達 **10 點** 以上</span>
            - <span style='color:#FBBF24 !important;'>**{t_list[2]}**：總點數達 **30 點** 以上</span>
            - <span style='color:#FBBF24 !important;'>**{t_list[3]}**：總點數達 **60 點** 以上</span>
            - <span style='color:#FBBF24 !important;'>**{t_list[4]}**：總點數達 **100 點** 以上 (頂峰稱號)</span>
            """, unsafe_allow_html=True)
        
        with tc2:
            st.markdown("##### <span style='color:#FBBF24 !important;'>🔍 選擇所屬組長與回報同仁姓名查詢</span>", unsafe_allow_html=True)
            
            q_lead_col, q_mem_col = st.columns(2)
            with q_lead_col:
                q_selected_leader = st.selectbox("👤 選擇所屬組長：", leader_list, key="game_query_leader_select_v2")
            with q_mem_col:
                q_available_members = leader_member_mapping.get(q_selected_leader, [])
                if q_available_members:
                    q_selected_member = st.selectbox("🎯 選擇回報同仁姓名：", q_available_members, key="game_query_member_select_v2")
                else:
                    q_selected_member = None
                    st.warning("⚠️ 此組長無成員")
            
            if q_selected_member:
                p_info = all_players_data.get(q_selected_member, {"str": 0, "vit": 0, "agi": 0, "cha": 0, "avail_pts": 0})
                ...
                st.info(f"👤 組別：【{q_selected_leader}】 | 同仁：【{q_selected_member}】\n當前稱號：**{p_lvl}** | 總點數：**{p_tot} 點** (已分配: {p_str+p_vit+p_agi+p_cha} | 未分配: {p_pts})")
            else:
                st.info("💡 請選擇所屬組長與回報同仁姓名進行查詢。")
...
# ==========================================
        ...
        st.markdown(
            ...
            <div style="text-align:center; margin-bottom:2rem;">
                ...
                <p style="color:#9CA3AF;">選擇日期與人員進行當日手動扣分，並支援依組別、人員與日期篩選查詢扣分明細！</p>
            </div>
            ''',
            unsafe_allow_html=True
        )
        ...
        # ==========================================
        st.markdown("### ✍️ 新增手動扣分登記")
        with st.form("manual_deduct_form"):
            col_f1, col_f2 = st.columns(2)
            with col_f1:
                m_date = st.date_input("📅 選擇扣分日期：", value=today_tw, key="manual_deduct_date")
            with col_f2:
                m_leader = st.selectbox("👤 選擇組別 (組長)：", all_leaders_keys, key="manual_deduct_leader")
            
            # 根據選擇的組長動態帶出組員
            team_members = leader_member_mapping.get(m_leader, [])
            m_person = st.selectbox("🎯 選擇被扣分人員：", team_members if team_members else [""], key="manual_deduct_person")
            
            m_score = st.number_input("📉 扣分分數：", min_value=1, value=1, step=1, key="manual_deduct_score")
            ...
            m_pwd = st.text_input("🔒 請輸入授權密碼：", type="password", key="manual_deduct_pwd")
            
            submitted_manual = st.form_submit_button("💾 確認送出手動扣分", use_container_width=True)
            
            if submitted_manual:
                if m_pwd != "12345":
                    st.error("❌ 驗證失敗：密碼錯誤，無法完成扣分登記。")
                elif not m_person:
                    st.error("❌ 請選擇有效的被扣分人員。")
                else:
        ...
        # ==========================================
        # 區塊二：篩選組別 + 人員 + 日期觀看扣分明細
        # ==========================================
        st.markdown("### 🔍 扣分明細查詢與篩選")
        
        q_col1, q_col2 = st.columns(2)
        with q_col1:
            q_start_date = st.date_input("開始日期：", value=today_tw - timedelta(days=7), key="query_deduct_start")
        with q_col2:
            q_end_date = st.date_input("結束日期：", value=today_tw, key="query_deduct_end")
        ...
        if q_start_date > q_end_date:
            ...
            q_date_list = []
            cur_q = q_start_date
            while cur_q <= q_end_date:
                q_date_list.append(cur_q.strftime("%Y-%m-%d"))
                cur_q += timedelta(days=1)

            # 篩選條件：組別與人員
            all_filter_groups = ["🌐 全部組別"] + all_leaders_keys
            selected_filter_group = st.selectbox("👁️ 篩選組別：", all_filter_groups, key="filter_group_sel")

            if selected_filter_group == "🌐 全部組別":
                filter_staff_options = ["全部人員"] + all_staff_flat
            else:
                filter_staff_options = ["全部人員"] + leader_member_mapping.get(selected_filter_group, [])

            selected_filter_staff = st.selectbox("👤 篩選人員：", filter_staff_options, key="filter_staff_sel")

            if st.button("📊 載入扣分明細查詢結果", use_container_width=True, type="primary"):
                all_records = []
                
                for d_str in q_date_list:
                    try:
                        m_res = requests.get(f"{MANUAL_DEDUCT_URL}/{d_str}.json").json() or {}
                    except:
                        m_res = {}

                    if isinstance(m_res, dict):
                        for record_id, r_data in m_res.items():
                            if isinstance(r_data, dict):
                                r_leader = r_data.get("組別", "未分類")
                                r_person = r_data.get("人員") or r_data.get("人员", "未知")
                                r_score = int(r_data.get("扣分", 0))
                                r_reason = r_data.get("原因", "")
                                r_time = r_data.get("登記時間", "")
                                r_author = r_data.get("登記人", "")

                                # 進行篩選比對
                                match_group = (selected_filter_group == "🌐 全部組別" or r_leader == selected_filter_group)
                                match_staff = (selected_filter_staff == "全部人員" or r_person == selected_filter_staff)

                                if match_group and match_staff:
                                    all_records.append({
                                        "日期": d_str,
                                        "組別": r_leader,
                                        "姓名": r_person,
                ...
                if not df_records.empty:
                    df_records = df_records.sort_values(by="登記時間", ascending=False).reset_index(drop=True)
                    st.markdown("##### 📋 符合條件的手動扣分明細列表")
                    ...
                    st.info("ℹ️ 在指定的篩選條件與日期區間內，查無手動扣分明細紀錄。")
        ...
        with tab1:
            ...
            with c2:
                if filter_type == "我的組員":
                    ...
                    if isinstance(all_staff, str):
                        staff_list = [s.strip() for s in all_staff.split(",") if s.strip()]
                    elif isinstance(all_staff, (list, tuple, set)):
                        staff_list = [str(s) for s in all_staff]
                    elif isinstance(all_staff, dict):
                        staff_list = [str(k) for k in all_staff.keys()]
                    else:
                        staff_list = []
                    search_staff = st.selectbox("🌍 選擇全廠人員", ["顯示全部"] + sorted(staff_list), key="unique_sel_all")
                        ...
                        with st.expander(f"👩‍🔧 {person} ({len(group)} 項)", expanded=True):
                            for _, row in group.iterrows():
                                ...
                                with col1:
                                    ...
                                    st.markdown(f'<div class="t-meta" style="font-size: 1.5rem !important; color: #ffeb3b !important; font-weight: 900;">登記人: {row.get("登記人","-")} | 時間: {row["登記時間"]}</div>', unsafe_allow_html=True)
                                with col2:
...
# --- ⚙️ 編輯手工具清單 (修正 Duplicate ID 版本) ---
    ...
    # --- ⚙️ 設定管理 ---
        ...
        with st.form("config_form"):
            so = st.text_area("製令清單 (以逗號隔開)", ",".join(str(x) for x in order_list) if isinstance(order_list, list) else str(order_list))
            sl = st.text_area("組長清單 (以逗號隔開)", ",".join(str(x) for x in all_leaders) if isinstance(all_leaders, list) else str(all_leaders))
            ss = st.text_area("人員清單 (以逗號隔開)", ",".join(str(x) for x in all_staff) if isinstance(all_staff, list) else str(all_staff))
            sp = st.text_area("工序清單 (以逗號隔開)", ",".join(str(x) for x in process_list) if isinstance(process_list, list) else str(process_list))
            sm = st.text_area("組長對應工序 (組長:工序1,工序2)", "\n".join([f"{k}:{','.join(str(x) for x in v)}" if isinstance(v, list) else f"{k}:{v}" for k, v in process_map.items()]) if isinstance(process_map, dict) else str(process_map))
            staff_in = st.text_area("組長屬下人員 (組長:人員1,人員2)", "\n".join([f"{k}:{','.join(str(x) for x in v)}" if isinstance(v, list) else f"{k}:{v}" for k, v in staff_map.items()]) if isinstance(staff_map, dict) else str(staff_map))
...
# ==========================================
        ...
        if current_active:
            ...
            if uploaded_pdf is not None:
                if st.button("🚀 確定上傳並加入右側心智圖列表", use_container_width=True, key="btn_upload_submit_main"):
