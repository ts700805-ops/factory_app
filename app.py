...
# --- 🔧 人員手工具紀錄表 (完整合併資產總覽版) ---
        ...
        with tab1:
            ...
            c1, c2 = st.columns(2)
            with c1:
                f_type = st.radio("篩選範圍", ["我的組員", "全廠人員搜尋"], horizontal=True, key="f_scope")
            with c2:
                if f_type == "我的組員":
                    s_staff = st.selectbox("👤 選擇人員", ["顯示全組"] + sorted(my_team), key="sel_team")
                else:
                    s_staff = st.selectbox("🌍 選擇人員", ["顯示全部"] + sorted(list(all_staff)), key="sel_all")
            ...
            if user_tool_raw:
                ...
                for k, v in user_tool_raw.items():
                    d = v.copy(); d['db_id'] = k
                    d['類型'] = "資產工具" if "【資產】" in str(v.get('手工具名稱','')) else "一般工具"
                    t_list.append(d)
                df = pd.DataFrame(t_list)

                # 執行篩選邏輯
                if f_type == "我的組員":
                    df = df[df["人員"].isin(my_team)] if s_staff == "顯示全組" else df[df["人員"] == s_staff]
                else:
                    df = df if s_staff == "顯示全部" else df[df["人員"] == s_staff]

                if not df.empty:
                    # 下載按鈕
                    csv = df.to_csv(index=False).encode('utf-8-sig')
                    st.download_button("📄 匯出清點表", data=csv, file_name="工具清點.csv")

                    # CSS 樣式 (移除多餘間距)
                    st.markdown("""
                        <style>
                            .t-card { background: white; border-radius: 10px; padding: 12px; margin-bottom: 8px; border: 1px solid #fce7f3; }
                            .a-card { border-left: 8px solid #8b5cf6 !important; background: #f5f3ff !important; }
                            .t-name { font-weight: 800; color: #1f2937; }
                            .t-qty { color: #db2777; margin-left: 5px; }
                        </style>
                    """, unsafe_allow_html=True)

                    for person, group in df.groupby("人員"):
                        with st.expander(f"👩‍🔧 {person} ({len(group)} 項)", expanded=True):
                            for _, row in group.iterrows():
                                db_id = row['db_id']
                                is_a = "a-card" if row['類型'] == "資產工具" else ""
                                
                                # 工具卡片
                                st.markdown(f'<div class="t-card {is_a}">', unsafe_allow_html=True)
                                col1, col2 = st.columns([7.5, 2.5])
                                with col1:
                                    st.markdown(f'<div class="t-name">🛠️ {row["手工具名稱"]} <span class="t-qty">x {row["數量"]}</span></div>', unsafe_allow_html=True)
                                    st.markdown(f'<div style="color:gray; font-size:0.8rem;">{row.get("登記人","-")} | {row["登記時間"]}</div>', unsafe_allow_html=True)
                                with col2:
                                    b1, b2 = st.columns(2)
                                    with b1:
                                        if st.button("✏️", key=f"e_{db_id}"): edit_record_dialog(db_id, row['手工具名稱'], row['數量'], person)
                                    with b2:
                                        if st.button("🗑️", key=f"d_{db_id}"): delete_record_dialog(db_id, row['手工具名稱'])
                                st.markdown('</div>', unsafe_allow_html=True)
                else:
                    st.info("💡 目前無紀錄")
            else:
        ...
        with tab1:
            ...
            c1, c2 = st.columns(2)
            with c1:
                filter_type = st.radio("篩選範圍", ["我的組員", "全廠人員搜尋"], horizontal=True, key="tool_scope")
            with c2:
                if filter_type == "我的組員":
                    search_staff = st.selectbox("👤 選擇組員", ["顯示全組"] + sorted(my_team), key="select_my_team")
                else:
                    search_staff = st.selectbox("🌍 選擇全廠人員", ["顯示全部"] + sorted(list(all_staff)), key="select_all_factory")
                ...
                if not display_df.empty:
                    ...
                    st.markdown("""
                        <style>
                            .tool-card {
                                ...
                                margin-bottom: 8px; border: 1px solid #fce7f3;
                                box-shadow: 1px 1px 3px rgba(0,0,0,0.03);
                            }
                            .asset-card { border-left: 8px solid #8b5cf6 !important; background-color: #f5f3ff !important; }
                            .tool-name { color: #1f2937; font-weight: 800; font-size: 1rem; }
                            .tool-qty { color: #db2777; font-weight: 700; margin-left: 8px; }
                            .tool-info { color: #9ca3af; font-size: 0.8rem; margin-top: 4px; }
                        </style>
                    """, unsafe_allow_html=True)
                    ...
                    for person, group in display_df.groupby("人員"):
                        with st.expander(f"👩‍🔧 {person} 的工具袋 (共 {len(group)} 項)", expanded=True):
                            for _, row in group.iterrows():
                                db_id = row['db_id']
                                is_asset = "asset-card" if row['類型'] == "資產工具" else ""
                                
                                # 卡片開始 (移除上方多餘間距)
                                st.markdown(f'<div class="tool-card {is_asset}">', unsafe_allow_html=True)
                                info_col, btn_col = st.columns([7.5, 2.5])
                                
                                with info_col:
                                    st.markdown(f"""
                                        <div class="tool-name">🛠️ {row['手工具名稱']} <span class="tool-qty">x {row['數量']}</span></div>
                                        <div class="tool-info">登記人: {row.get('登記人','-')} | {row["登記時間"]}</div>
                                    """, unsafe_allow_html=True)
                                ...
                                with btn_col:
                                    ...
                                    with b1:
                                        if st.button("✏️", key=f"edit_{db_id}"):
                                            edit_record_dialog(db_id, row['手工具名稱'], row['數量'], person)
                                    with b2:
                                        if st.button("🗑️", key=f"del_{db_id}"):
                                            delete_record_dialog(db_id, row['手工具名稱'])
                                st.markdown('</div>', unsafe_allow_html=True)
                else:
                    st.info("💡 目前沒有領用紀錄。")
            else:
        ...
        with tab2:
            ...
            if asset_tools_raw:
                ast_df = pd.DataFrame([{"資產編號": v.get("no"), "資產名稱": v.get("name"), "管理人員": v.get("管理人員", "系統"), "建立時間": v.get("建立時間", "-")} for v in asset_tools_raw.values()])
        ...
        with tab1:
            if user_tool_raw:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    filter_type = st.radio("篩選範圍", ["我的組員", "全廠人員搜尋"], horizontal=True, key="tool_filter_type")
                with c2:
                    if filter_type == "我的組員":
                        search_staff = st.selectbox("👤 選擇組員", ["顯示全組"] + sorted(my_team), key="tool_select_my_team")
                        display_df = tool_df[tool_df["人員"].isin(my_team)] if search_staff == "顯示全組" else tool_df[tool_df["人員"] == search_staff]
                    else:
                        search_staff = st.selectbox("🌍 選擇全廠人員", ["顯示全部"] + sorted(list(all_staff)), key="tool_select_all")
                        display_df = tool_df if search_staff == "顯示全部" else tool_df[tool_df["人員"] == search_staff]
                ...
                if not display_df.empty:
                    ...
                    st.download_button(label="📄 匯出含資產之清點表", data=csv_data, file_name=f"工具清點_{get_now_str()}.csv", mime="text/csv", key="tool_download_csv")
                ...
                st.markdown("""
                    <style>
                        .tool-card {
                            ...
                            margin-bottom: 10px; border: 1px solid #fce7f3;
                            box-shadow: 2px 2px 5px rgba(0,0,0,0.05);
                        }
                        .asset-card { border-left: 10px solid #8b5cf6 !important; background-color: #f5f3ff !important; }
                        .tool-name { color: #1f2937; font-weight: 800; font-size: 1.1rem; }
                        .tool-qty { color: #db2777; font-weight: 700; margin-left: 10px; }
                        .tool-info { color: #9ca3af; font-size: 0.85rem; margin-top: 5px; }
                    </style>
                """, unsafe_allow_html=True)
                    ...
                    for person, group in display_df.groupby("人員"):
                        with st.expander(f"👩‍🔧 {person} 的工具袋 (共 {len(group)} 項)", expanded=True):
                            for _, row in group.iterrows():
                                ...
                                tool_name = row['手工具名稱']
                                tool_qty = row['數量']
                                is_asset_class = "asset-card" if row['類型'] == "資產工具" else ""

                                st.markdown(f'<div class="tool-card {is_asset_class}">', unsafe_allow_html=True)
                                info_col, btn_col = st.columns([7.5, 2.5])
                                
                                with info_col:
                                    st.markdown(f"""
                                        <div class="tool-name">🛠️ {tool_name} <span class="tool-qty">x {tool_qty}</span></div>
                                        <div class="tool-info">
                                            <span style="color:#6d28d9;">[{row['類型']}]</span> | 
                                            登記人: {row.get('登記人','-')} | 時間: {row["登記時間"]}
                                        </div>
                                    """, unsafe_allow_html=True)
                                ...
                                with btn_col:
                                    ...
                                    with sub_c1:
                                        if st.button("✏️", key=f"main_edit_{db_id}"):
                                            edit_record_dialog(db_id, tool_name, tool_qty, person)
                                    with sub_c2:
                                        if st.button("🗑️", key=f"main_del_{db_id}"):
                                            delete_record_dialog(db_id, tool_name)
                                st.markdown('</div>', unsafe_allow_html=True)
                else:
                    st.info("💡 目前沒有符合條件的領用紀錄。")
            else:
                st.info("🌸 系統目前沒有領用紀錄喔！")
        ...
        with tab2:
            ...
            if asset_tools_raw:
                ...
                st.download_button("📄 匯出全廠資產清單", data=csv_ast, file_name=f"全廠資產表_{get_now_str()}.csv", key="asset_download_csv")
            else:
        ...
        with tab1:
            if user_tool_raw:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    filter_type = st.radio("篩選範圍", ["我的組員", "全廠人員搜尋"], horizontal=True, key="filter_radio")
                with c2:
                    if filter_type == "我的組員":
                        search_staff = st.selectbox("👤 選擇組員", ["顯示全組"] + sorted(my_team), key="select_my_team")
                        display_df = tool_df[tool_df["人員"].isin(my_team)] if search_staff == "顯示全組" else tool_df[tool_df["人員"] == search_staff]
                ...
                if not display_df.empty:
                    ...
                    export_df["清點數量"] = "" 
                    csv_data = export_df.to_csv(index=False).encode('utf-8-sig')
                    st.download_button(label="📄 匯出含資產之清點表", data=csv_data, file_name=f"工具清點_{get_now_str()}.csv", mime="text/csv", key="download_csv")

                # --- 顯示列表 ---
                st.markdown("""<style>.tool-row { padding:10px; margin-bottom:5px; border-radius:5px; border:1px solid #eee; } .asset-row { border-left: 8px solid #8b5cf6 !important; background-color: #f5f3ff !important; }</style>""", unsafe_allow_html=True)
                if not display_df.empty:
                    for person, group in display_df.groupby("人員"):
                        with st.expander(f"👩‍🔧 {person} 的工具袋 (共 {len(group)} 項)", expanded=True):
                            for _, row in group.iterrows():
                                ...
                                if b_c2.button("✏️", key=f"edit_btn_{row['db_id']}"):
                                    edit_record_dialog(row['db_id'], row['手工具名稱'], row['數量'], person)
                                if b_c3.button("🗑️", key=f"del_btn_{row['db_id']}"):
                                    delete_record_dialog(row['db_id'], row['手工具名稱'])
                                st.markdown("<div style='height:10px;'></div>", unsafe_allow_html=True)
                else:
                    st.info("💡 目前沒有符合篩選條件的領用紀錄。")
            else:
                st.info("🌸 系統目前沒有領用紀錄喔！")
        ...
        with tab2:
            ...
            if asset_tools_raw:
                ...
                csv_ast = ast_df.to_csv(index=False).encode('utf-8-sig')
                st.download_button("📄 匯出全廠資產清單", data=csv_ast, file_name=f"全廠資產表_{get_now_str()}.csv", key="download_asset_csv")
            else:
                st.info("💡 目前資產庫中沒有任何資料。")
