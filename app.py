            ...
            if data:
                ...
                if filtered_df.empty:
                    ...
                    for _, row in filtered_df.iterrows():
                        ...
                        with btn_col1:
                            if st.button(f"✅ 完工", key=f"done_{row['db_key']}"):
                                ...
                                st.rerun()
                        with btn_col2:
                            if st.button(f"📝 編輯", key=f"edit_btn_{row['db_key']}"):
                                st.session_state.target_edit_id = row['db_key']
                                st.info("請點選左側選單『📝 編輯派工紀錄』進行修改")
            else:
                st.info("目前尚無待辦派工。")
        ...
        try:
            ...
            if db_data:
                ...
                with st.form("edit_form"):
                    ...
                    e_proc = st.selectbox("修改製造工序", settings.get("processes", []), index=settings.get("processes", []).index(curr['製造工序']) if curr['製造工序'] in settings.get("processes", []) else 0)
                    ...
                    if st.form_submit_button("💾 儲存修改"):
                        requests.patch(f"{DB_URL}/{target_id}.json", json={"製令": e_order, "製造工序": e_proc, "派工人員": e_assigner, "作業人員": e_worker})
                        st.success("修改成功")
